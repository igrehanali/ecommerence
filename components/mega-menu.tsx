...
import { ChevronRight, Laptop, Smartphone, Headphones, Watch, Tablet, Monitor, Cpu, Speaker } from "lucide-react"
              ...
              {categories.map((category) => (
                <button
                  ...
                  className={`w-full text-left p-2 rounded flex items-center justify-between ${
                    activeCategory === category.id ? "bg-white bg-opacity-10" : ""
                  }`}
          ...
          <div className="p-4 border-b">
            <div className="flex items-center justify-between">
              <h2 className="font-medium text-lg">
                {categories.find((c) => c.id === activeCategory)?.name || "MacBook"}
              </h2>
            ...
            <div className="space-y-3">
              {categories
                ...
                ?.subcategories?.map((subcategory) => (
                  <div key={subcategory.id} className="hover:text-indigo-600 cursor-pointer">
                    {subcategory.name}
                  </div>
                ))}
            </div>
